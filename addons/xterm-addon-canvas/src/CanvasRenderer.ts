...
export class CanvasRenderer extends Disposable implements IRenderer {
  ...
  constructor(
    ...
    this._renderLayers = [
      new TextRenderLayer(this._screenElement, this._parentWindow, 0, this._colors, allowTransparency, this._id, this._bufferService, this._optionsService, characterJoinerService, decorationService),
      new SelectionRenderLayer(this._screenElement, this._parentWindow, 1, this._colors, this._id, this._bufferService, coreBrowserService, decorationService, this._optionsService),
      new LinkRenderLayer(this._screenElement, this._parentWindow, 2, this._colors, this._id, linkifier2, this._bufferService, this._optionsService, decorationService),
      new CursorRenderLayer(this._screenElement, this._parentWindow, 3, this._colors, this._id, this._onRequestRedraw, this._bufferService, this._optionsService, coreService, coreBrowserService, decorationService)
    ];
    this.dimensions = {
      scaledCharWidth: 0,
      scaledCharHeight: 0,
    ...
    this._devicePixelRatio = this._parentWindow.devicePixelRatio;
    this._updateDimensions();

    this.register(observeDevicePixelDimensions(this._renderLayers[0].canvas, this._parentWindow, (w, h) => this._setCanvasDevicePixelDimensions(w, h)));
  ...
  public onDevicePixelRatioChange(): void {
    ...
    if (this._devicePixelRatio !== this._parentWindow.devicePixelRatio) {
      this._devicePixelRatio = this._parentWindow.devicePixelRatio;
  ...
  private _updateDimensions(): void {
    ...
    this.dimensions.scaledCharWidth = Math.floor(this._charSizeService.width * this._parentWindow.devicePixelRatio);
